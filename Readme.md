...
while проверка:           # проверка цикла
  ...
  ```
  `break` - переходит за пределы ближайщего заключающего цикла (после всего оператора цикла)
...
8 6 4 2 0 
```
  `continue` аналог `goto` негативно влияет на читабельность кода. Нудно его аккуратнее использовать.
