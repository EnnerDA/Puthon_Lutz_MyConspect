...
```
Используй `dir`, чтобы получить список всех атрибутов — физически присутствующих и унаследованных и затем yflj применять либо `getattr` для извлечения их значений из экземпляра. Т.к. `__dict__` не отразить наследованые атрибуты ну и соответсвенно `экземпляр.__dict__[key]` не сработает. Посему `dir` и `getattr`.
