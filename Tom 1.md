...
Сумма элементов списка рекурсией в одну строку!!!Ну ладно в две, ее еще объявить же надо.
```python
  >>> def s(l):
	return 0 if not l else l[0] + s(l[1:])
```
Или вот с расширенным присвоением последовательности. Красивое!
  ```python
  >>> def s2(n):
	first, *rest = n # вот тут прямо хорошо стало
	return first if not rest else first + s2(rest)
  ```
  Python щграничивает количество рекурсий 1000 циклов, но мы можем это изменить
```python
>>> sys.getrecursionlimit()
1000
  ```
   К функциям можно прикреплять произвольные атрибуты.
```python
>>> def f(a):
return [x for x in dir(a) if x[:2] != '__']
>>> f(f)
[]
>>> f.atr = 'some atribute'
>>> f.count = 1
...
'some atribute'
```
Анотации к аргументам функции можно указывать во время описания функции используя следующий синтаксис:
```python
>>> def k(a: 'spam', b:(1,10)): # после двоеточия указанна аннотация, она не влияет на работу функции
>>> def k(a: 'spam' = 115, b:(1,10)): # можно совмещать и аннотации и станлартное значение
>>> k.__annotations__
{'a': 'spam', 'b': (1, 10)}
  ```
  **lambda функция**
  
  `lambda аргумент1, аргумент2, ..., аргументN: выражение, использующее эти аргументы`
```python  
...
105
 ```
**`map` отображение функции на элементы итерируемого объекта**
```python
>>> def f(x): return x**2 # функция которую надо применить к элементам
>>> l = [1,2,3,4,5]       # итерируемый объект
>>> list(map(f,l))        # синтаксис map
[1, 4, 9, 16, 25]
```
+ lambda:
```python
...
[1, 4, 9, 16, 25]
  ```
Аналог списковым включением
  ```python
>>> [x**2 for x in l]
[1, 4, 9, 16, 25]
  ```
  **`filter` - выбирает элементы из итерируемого объекта на основе проверочной функции**
```python
...
[1, 2, 3, 4]
  ```
   **`reduce` - комбинируем элементы из итерируемого объекта и применяет к ним переданную функцию** вычисляет единственное значение путем применения функции к накопителю и следующим друг за другом элементам.	
...
Сравним `map` и списковое вклчение в простых случаях
```python
>>> l = list(map(lambda x: x**2, range(11)))
>>> l2 = [x**2 for x in range(11)]
>>> l2 == l
True
```
Сравним с `filter`
```python
>>> l3 = [x for x in range(5) if not x%2]
>>> l4 = list(filter((lambda x: x%2 == 0), range(5)))
>>> l3 == l4
True
```
Списковое включение чуть короче и приятнее для чтения.
Если же нам требуется совместить и `map` и `filter`, то списковое включение сильно выйграет
```python
l5 = [x**2 for x in range(5) if not x%2]
```
Списковые включения поддерживают любое количество вложенны циклов `for` с условиями
```
[выражение for цель1 in итерируемый объект1 if условие1
    for цель2 in итерируемый объект2 if условие2 ...
    for цельN in итерируемый объектN if условиеN]
```
```python
...
Та же логика снитаксиса применима к включениеям словарей множеств и генераторным выражениям.
Списковые включения для просмотра матриц
```python
>>> M = [[1,2,3],
...      [4,5,6],
...      [7,8,9]]
>>> M[2] # третяя строка
[7, 8, 9]
>>> [row[1] for row in M] # второй столбец
[2, 5, 8]
[M[i][i] for i in range(len(M))] # диагональ
[1, 5, 9]
>>> [el + 10 for row in M for el in row] # добавить 10 к каждому элементу
...
['hamp', 'amph', 'mpha', 'pham']
```
Напишем свою `map` с блэкджэком и прочими
```python
>>> def mymap(f,*seq):
	res = []
	for x in zip(*seq):
		res.append(f(*x))
	return res
>>> mymap(pow, [1,2,3,4], [5,6,7,8])
[1, 64, 2187, 65536]
>>> mymap(print, [1,2,3,4], [5,6,7,8])
1 5
2 6
3 7
4 8
...
Виды включений
```python
>>> [x*x for x in range(10)] # списковое включение
[0, 1, 4, 9, 16, 25, 36, 49, 64, 81]
>>> (x*x for x in range(10)) # генератор
<generator object <genexpr> at 0x0000000002F71F20>
...
1. домашний каталог сценария верхнего уровня (каталог, содержащий его)
2. все каталоги, перечисленные в переменной среды PYTHONPATH 
3. каталоги стандартной библиотеки
4. все каталоги внутри файлов конфигурации путей .pth, находящихся в стандартных местах
5. подкаталог site-packages для установленных сторонних расширений
  
Смену поиска модулей в рамках выполнения одного сценария отдлично реализует следующий код:
```python 
import sys
sys.path.append('c:\...')
```
  Отличный способ добавить каталог в стандартный поиск это сделать файл `myconfig.pth` прописать в нем путь к каталогу который нам нужен и поместить его в `\Python38\Lib\site-packages`. Теперь наш каталог добавлен в пути стандартного поиска.
  
Импортнуть можно:
  1. файл кода Python .py
  2. файл байт-кода .pyc, .pyo
  3. модуль расширения С .so, .dll (Linux), .dll (Windows) 
  4. каталог для импортирования пакетов
  5. zip, java, .net, и прочие произвольные компоненты
  
  ## Глава 23. Основы написания модулей.
  
При использовании конструкции `from module1 import function1` происходит полная загрузка модуля, в область имен попадает `function1` но остальной код написанный в `module1` будет выполнен. Аккуратнее с этим!

Как функции не могут видеть имена в других функциях, если только они физически не вкладываются в них; так и **код модуля не может видеть имена в других модулях**, если только он явно не	импортирует их.

Функция `imp.reload` вынуждает код уже загруженного модуля перезагрузиться и повторно выполниться.
```python
import module
...использовать атрибуты модуля
from imp import reload
reload(module) # reload - функция а не оператор посему пишем скобочки
```
`reload` не перегружает то, что было импортировано через `from`. Аккуратнее с этим парниша.

## Глава 24. Пакеты модулей.

**Импортирование пакетами.**
  
`import dirl.dir2.mod` если нужный модуль `mod.py` имеет такой путь `dir0\dirl\dir2\mod.py` Контейнерный каталог `diг0` должен быть добавлен в путь поиска модулей при условии, что он не представляет собой домашний каталог файла верхнего уровня, в точности как если бы имя `dir1` обозначало простой файл модуля. 

  При этом доступ к атрибутe, например, `x` модуля будет таков `dirl.dir2.mod.x`.
  Если же мы импортируем омдуцль через `from`, то
  ```python
  from dirl.dir2.mod import x
  x
  ```
Импортирование модулей расположенных внутри того же самого пакета называется операциями *относительного импортирования.*

  Импортирование модулей, находящихся где-то в другом месте в пути поиска импортируемых модулей	называется операциями *абсолютного импортирования.*	
  
  Импортирование с ведущими точками осуществляет относитеьное импортирование. Без точек - пропускает относительные компоненты внутри пути поиска импортируемых модулей и взамен ищет в абсолютных каталогах, которые содержатся в `sys.path`.	
```python
from . import string  # Искать только в этом пакете
from import string    # Искать сначала в стандартной библиотеке, то шо перечислено в  sys.path
```
```python
  # в коде модуля А.В.С
from . import D 	  # Импортирует A.B.D (. означает A. В) 	
from .. import E   	# Импортирует A.E (. . означает A) 	
from . D import X 	# Импортирует A.B.D.X (. означает A. В) 	
from .. E import X 	# Импортирует A.E.X (. . означает A)
```
Оперции относительньо импортирования действуют внутри файлов модулей используемых как часть пакета. 

  Нормальные операции импортирования в файлах, не используемых в качестве части пакета, автоматически выполняют поиск сначала в каталоге, который содержит сценарий верхнего уровня.
  
Далее тема раскрыта отвратительно, то ли автор то ли переводчик видимо перед написанием этой части главы питались исключительно костями в течнеие 27 лет, я смог дочитать это до резюме и ответить на все вопросы кроме последнего в конце, но о чем там я понятия не имею. Наверно попробую прочитать еще раз только в случае полнейщего тупика и второго пришествия случившихся единомоментно.
  
## Глава 25. Расширенные возможности модулей.

Если присвоить переменной `__all__` список имен переменных, то при импорте этого модуля через `from import *` скопируются только те, кто перечислен в `__all__`.
```pythin
#alls.py
__all__ = ['a', '_c']
a, b, _c, _d = 1, 2, 3, 4
...
```
**`__name__ == __main__`**
Если файл запускается как программа верхнего уровня, то `__name__ == '__main__'`, если файл запускается как модуль то `__name__ == 'имя модуля'`
```python
#anyfile
def nameprint():
    print(__name__)
...
anyfile
```
Так удобно организовывать проверку файла, упоковав в ветвление `if __name__ == '__main__':`` код будет выполняться только в случае прямого открытия файла. А если его импортировать, то тестовый код не выполнится.

[formats.py](https://github.com/EnnerDA/Puthon_Lutz_MyConspect/blob/main/formats.py)

Опять он про свой Unicod заладил:
```python
